...
import {
  BookOpen, FileText, CalendarCheck, BarChart2,
  CheckCircle2, Clock, XCircle, AlertCircle,
  Loader2, Bell, Shield, GraduationCap,
  TrendingUp, Zap, Hourglass, LogIn, LogOut,
  Timer, User, Rocket, ThumbsUp, ThumbsDown,
  Construction,
} from "lucide-react";
...
if (typeof document !== "undefined" && !document.getElementById("__ed_s__")) {
  const el = document.createElement("style"); el.id="__ed_s__"; el.textContent=_S; document.head.appendChild(el);
}
...
/* ─── Status colours ─── */
const STATUS_DOT = {
  Present: "bg-emerald-500", Late: "bg-amber-400",
  Early:   "bg-sky-400",     Absent: "bg-red-500", Leave: "bg-slate-400",
};
const STATUS_TEXT = {
  ...
  return (
    <div className="ed-card p-6 relative overflow-hidden">
      ...
      <div className="relative flex flex-col items-center justify-center py-8 gap-3">
        ...
        <p className="text-sm text-gray-500 text-center leading-relaxed max-w-[200px]">{description}</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="ed-card px-5 py-5 flex items-center gap-4 ed-in">
      ...
      <div className="min-w-0">
        ...
        {sub && !loading && <p className="text-[11px] text-gray-400 mt-0.5 truncate">{sub}</p>}
      </div>
    </div>
  );
}
...
function AttendanceHeatmap({ records, loading }) {
  const [tooltip, setTooltip] = useState(null); // { x, y, date, status, hours }
  const [hoveredDate, setHoveredDate] = useState(null);

  /* Build a full-year grid: last 12 months → 53 weeks × 7 days */
  const { weeks, monthMarkers } = useMemo(() => {
    ...
    const weeks       = [];
    const monthMarkers= []; // { colIdx, label }
    let   col         = [];
    let   colIdx      = 0;
    let   lastMonth   = -1;
    ...
    while (cur <= today || col.length % 7 !== 0) {
      ...
      /* Month label: mark when month changes on a Sunday */
      ...
      let   status;
      if (isFut)       status = "Future";
      else if (isWkd)  status = "Weekend";
    ...
    <div>
            ...
            week.map((day, di) => {
              ...
              return (
                <rect
                  ...
                  style={{ cursor: day.isFut || day.isWkd ? "default" : "pointer", transition: "opacity .1s" }}
                  onMouseEnter={e => {
                    if (day.isFut) return;
      ...
      <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 mt-3">
        {Object.entries(STATUS_COLORS)
          .filter(([k]) => !["Weekend","Future","Empty"].includes(k))
          .map(([status, { bg, label }]) => (
            <span key={status} className="flex items-center gap-1.5 text-[11px] text-gray-500">
              <span className="w-3 h-3 rounded-sm shrink-0" style={{ background: bg }} />
              {label}
            </span>
          ))}
      ...
      {tooltip && (
        ...
        >
          <div className="ed-card px-3 py-2.5 text-xs shadow-xl border border-gray-100 min-w-[140px]">
            <p className="font-bold text-gray-800 mb-1">{tooltip.date}</p>
            <div className="flex items-center gap-1.5 mb-0.5">
              <span className="w-2 h-2 rounded-full shrink-0"
                style={{ background: STATUS_COLORS[tooltip.status]?.bg || "#94a3b8" }} />
              <span className="font-semibold" style={{ color: STATUS_COLORS[tooltip.status]?.bg }}>
                {tooltip.status}
              </span>
            </div>
            {tooltip.in  && <p className="text-gray-400">In: <span className="text-gray-700 font-medium">{fmt12(tooltip.in)}</span></p>}
            {tooltip.out && <p className="text-gray-400">Out: <span className="text-gray-700 font-medium">{fmt12(tooltip.out)}</span></p>}
            {tooltip.hours && <p className="text-gray-400">Hours: <span className="text-gray-700 font-medium">{fmtHours(tooltip.hours)}</span></p>}
          </div>
        </div>
      )}
    </div>
  ...
  return (
    <div className="flex items-center justify-between py-2.5 border-b border-gray-50 last:border-0">
      <div className="flex items-center gap-2.5 min-w-0">
        ...
        <div className="min-w-0">
          <p className="text-sm font-semibold text-gray-800 truncate">{app.type}</p>
          <p className="text-[10px] text-gray-400">{app.from_date} → {app.to_date}</p>
        </div>
      </div>
      <span className={`flex items-center gap-1 text-[10px] font-bold px-2 py-1 rounded-full shrink-0 ml-2 ${cfg.cls}`}>
    ...
    <div className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
      <div className="flex items-center gap-2.5">
        ...
        <div>
          ...
          <p className="text-[10px] text-gray-400">{fmt12(rec.in_time)} – {fmt12(rec.out_time)}</p>
        </div>
      </div>
      <div className="flex items-center gap-2">
        {rec.hours && <span className="text-[10px] text-gray-400 font-medium">{fmtHours(rec.hours)}</span>}
        <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${cfg}`}>{rec.status}</span>
      </div>
    </div>
  );
}
...
export default function EmployeeDashboard() {
  ...
  const fetchToday = useCallback(async () => {
    ...
    try {
      const base = typeof API.MyAttendanceToday === "function"
  ...
  const fetchSummary = useCallback(async () => {
    ...
    try {
      const base = typeof API.MyAttendanceSummary === "function"
  ...
  const fetchRecords = useCallback(async () => {
    ...
    try {
      ...
      const results = await Promise.all(months.map(async month => {
        try {
          const base = typeof API.MyAttendanceByMonth === "function"
  ...
  const firstName    = (auth?.name || "").split(" ")[0] || "Employee";
    ...
    <div className="ed-page min-h-screen bg-[#f8fafc] px-5 sm:px-8 py-8 text-gray-900">
      ...
      <header className="mb-8 ed-in">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          ...
          {/* Today's status pill */}
          <div className={`flex items-center gap-2 px-4 py-2.5 rounded-2xl border text-sm font-semibold self-start sm:self-auto ${
            todayLoad ? "bg-gray-50 text-gray-400 border-gray-200 ed-pulse" :
            todayStatus === "Present" ? "bg-emerald-50 text-emerald-700 border-emerald-200" :
            todayStatus === "Late"    ? "bg-amber-50 text-amber-700 border-amber-200"    :
            "bg-gray-50 text-gray-500 border-gray-200"
          }`}>
            <span className={`w-2 h-2 rounded-full ${
              todayLoad ? "bg-gray-300" :
              todayStatus === "Present" ? "bg-emerald-500" :
              todayStatus === "Late"    ? "bg-amber-500"   : "bg-gray-400"
            }`} />
      ...
      <section className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-7 ed-stagger">
        ...
        <StatWidget
          ...
          Icon={TrendingUp}
          color={
            attendRate == null ? "bg-gray-100 text-gray-500" :
            attendRate >= 90   ? "bg-emerald-100 text-emerald-600" :
            attendRate >= 75   ? "bg-amber-100 text-amber-600" :
        ...
        <StatWidget
          ...
          sub={pendingApps === 1 ? "1 awaiting approval" : `${pendingApps} awaiting approval`}
          Icon={Hourglass}
      ...
      {!summLoad && summary && (
        <div className="flex flex-wrap gap-2 mb-7 ed-in">
          {[
            ...
            <span key={label} className="flex items-center gap-1.5 px-3 py-1.5 bg-white border border-gray-100 rounded-full text-xs font-semibold text-gray-700 shadow-sm">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-5">
        ...
        <Card title="Today's Detail">
          {todayLoad ? (
            ...
            <div className="space-y-3">
              {[
                { label: "Status",    value: <span className={`text-xs font-bold px-2.5 py-1 rounded-full border ${STATUS_TEXT[todayRec.status] || "bg-gray-50 text-gray-600 border-gray-200"}`}>{todayRec.status}</span> },
                { label: "In",        value: <span className="text-sm font-bold text-gray-900">{fmt12(todayRec.in_time)}</span>  },
                { label: "Out",       value: <span className="text-sm font-bold text-gray-900">{fmt12(todayRec.out_time)}</span> },
                { label: "Hours",     value: <span className="text-sm font-bold text-gray-900">{fmtHours(todayRec.hours)}</span> },
                { label: "Mode",      value: <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${todayRec.attendance_mode === "remote" ? "bg-teal-50 text-teal-700" : "bg-gray-100 text-gray-600"}`}>{todayRec.attendance_mode || "onsite"}</span> },
              ].map(({ label, value }) => (
            ...
            <div className="flex flex-col items-center justify-center py-10 text-center">
              ...
              <p className="text-sm text-gray-400">No punch record for today.</p>
              <p className="text-xs text-gray-300 mt-1">This updates automatically after machine sync.</p>
            </div>
          )}
        </Card>
        ...
        {!appLoad && appStats && (
          <Card title="Application Breakdown">
            <div className="space-y-3">
              {[
                { label: "Pending",      val: appStats.pending,      color: "#7c3aed", bg: "bg-violet-500" },
                { label: "HOD Approved", val: appStats.hod_approved, color: "#0284c7", bg: "bg-sky-500"    },
                { label: "Approved",     val: appStats.approved,     color: "#059669", bg: "bg-emerald-500"},
                { label: "Rejected",     val: (appStats.rejected || 0) + (appStats.hod_rejected || 0), color: "#dc2626", bg: "bg-red-500" },
                  ...
                  <div key={label}>
                    ...
                    <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
                      <div className={`h-1.5 rounded-full ${bg}`}
                        style={{ width: `${pct}%`, transition: "width .6s cubic-bezier(.4,0,.2,1)" }} />
                    </div>
                  </div>
                );
              })}
              <p className="text-[10px] text-gray-400 pt-1">{appStats.total} total application{appStats.total !== 1 ? "s" : ""}</p>
            </div>
          </Card>
        )}
