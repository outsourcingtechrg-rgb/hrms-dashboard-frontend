...
import {
  X, Search, Shield, PlusCircle, Edit3, Trash2, Eye,
  Send, RefreshCw, ChevronLeft, ChevronRight,
  CheckCircle2, XCircle, Loader2, AlertTriangle,
  Crown, Key, Users, Lock, ChevronDown, Check,
  ChevronRight as ChevronRightSmall, Info, Hash, Calendar,
} from "lucide-react";
...
const ALL_PERMISSION_KEYS = Object.values(PERMISSION_GROUPS).flat().map(p => p.key);

/* ─── Level → badge style ─── */
const LEVEL_CFG = [
  { color: "bg-red-50 text-red-700 border-red-200",         dot: "bg-red-500",    label: "Super Admin" },
  { color: "bg-amber-50 text-amber-700 border-amber-200",   dot: "bg-amber-500",  label: "Admin"       },
  { color: "bg-violet-50 text-violet-700 border-violet-200",dot: "bg-violet-500", label: "Manager"     },
  { color: "bg-blue-50 text-blue-700 border-blue-200",      dot: "bg-blue-500",   label: "HR"          },
  { color: "bg-green-50 text-green-700 border-green-200",   dot: "bg-green-500",  label: "Staff"       },
  { color: "bg-gray-50 text-gray-600 border-gray-200",      dot: "bg-gray-400",   label: "Viewer"      },
];

function getLevelCfg(level) {
  return LEVEL_CFG[Math.min(level - 1, LEVEL_CFG.length - 1)] || LEVEL_CFG[LEVEL_CFG.length - 1];
}
...
function Toast({ toast }) {
  ...
  return (
    <div className={`fixed bottom-6 left-1/2 -translate-x-1/2 z-500 flex items-center gap-3 px-5 py-3 rounded-2xl text-white shadow-xl text-sm font-medium ${bg} fade-in pointer-events-none`}>
...
function PermissionsEditor({ permissions = [], onChange }) {
  ...
  const toggleGroup = (perms) => {
    ...
    onChange(allOn ? permissions.filter(k => !keys.includes(k)) : [...new Set([...permissions, ...keys])]);
  };
    ...
    <div>
      <div className="flex items-center justify-between mb-2.5">
        ...
        <div className="flex gap-3">
          <button type="button" onClick={() => onChange([...ALL_PERMISSION_KEYS])}
            className="text-[11px] font-semibold text-blue-600 hover:underline">Select All</button>
          <span className="text-gray-300 text-xs">·</span>
          <button type="button" onClick={() => onChange([])}
            className="text-[11px] font-semibold text-gray-400 hover:text-red-500 hover:underline">Clear All</button>
        </div>
      </div>
      ...
      <div className="space-y-1.5 max-h-72 overflow-y-auto rounded-xl border border-gray-100 p-2 bg-gray-50/50">
        {Object.entries(PERMISSION_GROUPS).map(([group, perms]) => {
          ...
          return (
            <div key={group} className="rounded-xl overflow-hidden border border-gray-100 bg-white">
              {/* Group row */}
              <div
              ...
              >
                ...
                <span className="text-xs font-semibold text-gray-700 flex-1 leading-none">{group}</span>
                <span className="text-[10px] text-gray-400 font-medium tabular-nums">{selectedCount}/{keys.length}</span>
                <ChevronRightSmall size={12} className={`text-gray-400 transition-transform shrink-0 ${isOpen ? "rotate-90" : ""}`} />
              </div>
                  ...
                  {perms.map(perm => {
                    ...
                    return (
                      <label key={perm.key}
                        ...
                        <span className="text-xs leading-none">{perm.label}</span>
                      </label>
                    );
                  })}
                </div>
...
function RoleFormModal({ role, onClose, onSaved, showToast }) {
  ...
  const [form, setForm]       = useState(
    isEdit
      ? { name: role.name, level: role.level, permissions: role.extra_data?.permissions || [] }
      : { ...EMPTY_FORM }
  );
  ...
  const validate = () => {
    ...
    if (!form.level || form.level < 1 || form.level > 10) e.level = "Level must be between 1 and 10.";
    return e;
  };
    ...
    <div className="fixed inset-0 z-100 flex items-center justify-center bg-black/40 backdrop-blur-sm fade-in">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-xl mx-4 max-h-[92vh] flex flex-col overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-lg bg-blue-50 border border-blue-200 flex items-center justify-center">
              {isEdit ? <Edit3 size={14} className="text-blue-600" /> : <PlusCircle size={14} className="text-blue-600" />}
            </div>
            <div>
              <p className="font-semibold text-gray-900 text-sm leading-tight">{isEdit ? "Edit Role" : "Create New Role"}</p>
              <p className="text-[11px] text-gray-400 leading-tight mt-0.5">
                {isEdit ? `Editing: ${role.name}` : "Define a new role & its permissions"}
              </p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          ...
          {/* Name */}
          <div>
            <label className="text-[11px] uppercase tracking-widest font-semibold text-gray-400 mb-1.5 block">Role Name *</label>
            <input
              type="text"
              value={form.name}
          ...
          <div>
            <label className="text-[11px] uppercase tracking-widest font-semibold text-gray-400 mb-1.5 block">
              Access Level * <span className="normal-case text-gray-400 font-normal">(1 = highest authority)</span>
            </label>
            <div className="flex items-center gap-3">
              ...
              />
              <div className={`flex items-center gap-2 px-3 py-1.5 rounded-xl border text-xs font-semibold ${levelCfg.color}`}>
            ...
            <div className="mt-2 flex flex-wrap gap-1.5">
              {LEVEL_CFG.map((cfg, i) => (
                <button key={i} type="button" onClick={() => set("level", i + 1)}
        ...
        <div className="px-6 py-4 border-t border-gray-100 flex gap-3">
          <button onClick={handleSubmit} disabled={saving}
            ...
            {saving
              ? <><Loader2 size={15} className="spin" /> {isEdit ? "Saving…" : "Creating…"}</>
...
function RoleSidebar({ role, onClose, onEdit, onDelete }) {
  ...
  const grouped = Object.entries(PERMISSION_GROUPS).map(([group, items]) => ({
    group,
    ...
    <>
      ...
      <div className="fixed right-0 top-0 bottom-0 z-51 w-full sm:w-110 bg-white shadow-2xl flex flex-col slide-in-right">
        ...
        <div className="flex items-start justify-between px-6 py-4 border-b border-gray-100">
          <div>
            <div className="flex items-center gap-2 flex-wrap mb-1">
              <span className={`text-xs font-semibold px-2.5 py-1 rounded-lg border flex items-center gap-1.5 ${levelCfg.color}`}>
                <span className={`w-2 h-2 rounded-full ${levelCfg.dot}`} />
                Level {role.level} · {levelCfg.label}
              </span>
            </div>
            <h2 className="text-base font-semibold text-gray-900">{role.name}</h2>
            <p className="text-xs text-gray-400 mt-0.5">ID #{role.id} · Created {role.created_at ? new Date(role.created_at).toLocaleDateString() : "—"}</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
              ...
              <div className="space-y-2">
                {grouped.map(({ group, items }) => (
                  <div key={group} className="bg-gray-50 rounded-xl px-3 py-3">
                    <div className="text-[10px] uppercase tracking-widest font-semibold text-gray-500 mb-2">{group}</div>
          ...
          {role.extra_data && Object.keys(role.extra_data).filter(k => k !== "permissions").length > 0 && (
            <div>
              <div className="text-[10px] uppercase tracking-widest font-semibold text-gray-400 mb-2">Extra Data</div>
        ...
        <div className="px-6 py-4 border-t border-gray-100 flex gap-2">
          <button onClick={() => { onEdit(role); onClose(); }}
          ...
          <button onClick={() => { onDelete(role); onClose(); }}
  ...
  return (
    <div className="fixed inset-0 z-200 flex items-center justify-center bg-black/50 backdrop-blur-sm fade-in">
      <div className="bg-white rounded-2xl p-6 w-95 max-w-[calc(100vw-32px)] shadow-2xl">
        ...
        <h2 className="text-base font-semibold text-gray-900 text-center mb-1">Delete Role?</h2>
        <p className="text-sm text-gray-500 text-center mb-1">You are about to permanently delete:</p>
        <p className="text-sm font-bold text-gray-800 text-center mb-1">"{role.name}"</p>
        <p className="text-xs text-gray-400 text-center mb-5">Level {role.level} · ID #{role.id}</p>
        <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-2.5 flex items-start gap-2 mb-5">
          <AlertTriangle size={14} className="text-amber-600 mt-0.5 shrink-0" />
          <p className="text-xs text-amber-700">Employees assigned this role may lose access. This action cannot be undone.</p>
        </div>
        <div className="flex gap-3">
...
export default function RolesPage() {
  ...
  const fetchRoles = useCallback(async () => {
    ...
    setFetchError(null);
    try {
      const res = await fetch(API.AllRoles);
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      const res = await fetch(API.DeleteRoleByID(deleteTarget.id), { method: "DELETE" });
  ...
  const stats = {
    total:    roles.length,
    maxPerms: roles.reduce((m, r) => Math.max(m, (r.extra_data?.permissions||[]).length), 0),
  ...
  const filtered = roles
    .filter(r =>
      (!search || r.name.toLowerCase().includes(search.toLowerCase()) || String(r.id).includes(search)) &&
      (filterLevel === "All" || String(r.level) === filterLevel)
    )
    .sort((a, b) => a.level - b.level || a.name.localeCompare(b.name));

  const uniqueLevels = [...new Set(roles.map(r => r.level))].sort((a,b) => a-b);
    ...
    <div className="min-h-screen bg-gray-50 px-8 py-10 text-gray-900">
      ...
      <header className="mb-6 flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500">Define access levels and permissions for your organisation · Super Admin</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        <Widget title="Total Roles"       value={loading ? "…" : stats.total}   sub="Defined in system"         icon={Shield}    color="border-blue-300 bg-blue-50 text-blue-700"      />
        <Widget title="Access Levels"     value={loading ? "…" : stats.levels}  sub="Distinct levels in use"    icon={Key}       color="border-violet-300 bg-violet-50 text-violet-700" />
        <Widget title="Max Permissions"   value={loading ? "…" : stats.maxPerms} sub="In a single role"         icon={Lock}      color="border-amber-300 bg-amber-50 text-amber-700"   />
      </div>
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        {LEVEL_CFG.map((cfg, i) => (
          <button key={i}
            onClick={() => setFilter(() => setFilterLevel(filterLevel === String(i+1) ? "All" : String(i+1)))}
        ...
        {filterLevel !== "All" && (
          <button onClick={() => { setFilterLevel("All"); setPage(1); }}
      ...
      <div className="bg-white border border-gray-100 rounded-xl px-4 py-3 shadow-sm flex flex-wrap items-center gap-3 mb-4">
        ...
        <select value={filterLevel} onChange={e => setFilter(() => setFilterLevel(e.target.value))}
          ...
          {uniqueLevels.map(l => <option key={l} value={l}>Level {l} — {getLevelCfg(l).label}</option>)}
        </select>
        ...
        <span className="ml-auto text-xs text-gray-400">{filtered.length} role{filtered.length !== 1 ? "s" : ""}</span>
      </div>
      ...
      >
        ...
        {fetchError && (
          <div className="flex items-start gap-3 bg-red-50 border border-red-200 rounded-xl px-4 py-3 mb-4">
            ...
            <div>
              <p className="text-sm font-semibold text-red-700">Failed to load roles</p>
              <p className="text-xs text-red-500 mt-0.5">{fetchError}</p>
            </div>
            <button onClick={fetchRoles} className="ml-auto text-xs font-semibold text-red-600 hover:underline shrink-0">Retry</button>
          </div>
        )}
          ...
          <table className="min-w-full text-sm border-collapse">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-100">
                {["#", "Role Name", "Level", "Permissions", "Created", "Actions"].map((h, i) => (
                  <th key={h} className={`py-3 px-4 text-[11px] font-semibold uppercase tracking-wider text-gray-400 whitespace-nowrap
                    ${i <= 1 ? "text-left" : "text-center"}`}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {loading ? (
                <tr><td colSpan={6} className="py-16 text-center">
                  <Loader2 size={24} className="mx-auto text-blue-400 spin mb-3" />
                  <p className="text-sm text-gray-400">Loading roles…</p>
                </td></tr>
              ) : paginated.length === 0 ? (
                <tr><td colSpan={6} className="py-16 text-center">
                  <Shield size={30} className="mx-auto mb-3 text-gray-300" />
                  <p className="text-sm text-gray-400 mb-3">{search || filterLevel !== "All" ? "No roles match your filters." : "No roles found."}</p>
                  {!search && filterLevel === "All" && (
                    <button onClick={() => setShowCreate(true)} className="text-sm text-blue-600 font-medium hover:underline">+ Create first role</button>
                  )}
                </td></tr>
                ...
                return (
                  <tr key={role.id}
                    ...
                    {/* ID */}
                    <td className="py-3 px-4 text-xs text-gray-400 font-medium tabular-nums">#{role.id}</td>

                    {/* Name */}
                    <td className="py-3 px-4 cursor-pointer" onClick={() => setViewRole(role)}>
                      <div className="flex items-center gap-2.5">
                        <div className={`w-8 h-8 rounded-xl flex items-center justify-center border shrink-0 ${cfg.color}`}>
                          <Shield size={14} />
                        </div>
                        <span className="text-sm font-medium text-blue-600 hover:underline">{role.name}</span>
                      </div>
                    </td>
                      ...
                      <div className="flex items-center justify-center gap-2">
                        <div className="w-16 bg-gray-100 rounded-full h-1.5 overflow-hidden">
                          <div className="h-1.5 rounded-full bg-blue-400 transition-all"
                            style={{ width: `${(perms.length / ALL_PERMISSION_KEYS.length) * 100}%` }} />
                        </div>
                        <span className="text-xs font-medium text-gray-600 tabular-nums">{perms.length}</span>
                      </div>
                    </td>
                    ...
                    <td className="py-3 px-4 text-center">
                      <div className="flex items-center justify-center gap-1">
                        <button onClick={() => setViewRole(role)}
      ...
      {viewRole && (
        <RoleSidebar
          ...
          onDelete={r => { setViewRole(null); setDeleteTarget(r); }}
        />
      )}
      {deleteTarget && (
        <DeleteConfirm
