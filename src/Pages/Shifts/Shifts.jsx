...
if (
  ...
  !document.getElementById("__shifts_styles__")
) {
  const el = Object.assign(document.createElement("style"), {
    id: "__shifts_styles__",
    textContent: _S,
  });
...
const MOCK_SHIFTS = [
  {
    id: 1,
    name: "Morning Shift",
    shift_start_timing: "08:00:00",
    shift_end_timing: "16:00:00",
    shift_late_on: "08:30:00",
  ...
  },
  {
    ...
    shift_start_timing: "12:00:00",
    shift_end_timing: "20:00:00",
    shift_late_on: "12:20:00",
    total_hours: "08:00:00",
    saturday_on: true,
    allow_remote: true,
    extra_data: null,
  ...
  },
  {
    id: 3,
    name: "Night Shift",
    shift_start_timing: "22:00:00",
    shift_end_timing: "06:00:00",
    shift_late_on: "22:15:00",
    total_hours: "08:00:00",
    saturday_on: true,
    allow_remote: false,
    extra_data: null,
  ...
  },
  {
    id: 5,
    name: "Remote Flex",
    shift_start_timing: "10:00:00",
    shift_end_timing: "18:00:00",
    shift_late_on: "10:30:00",
    total_hours: "08:00:00",
    saturday_on: false,
    allow_remote: true,
    extra_data: null,
...
function Toggle({ on, onChange, label }) {
  return (
    ...
    >
      <span
        className={`inline-block w-4 h-4 bg-white rounded-full shadow transition-transform ${on ? "translate-x-6" : "translate-x-1"}`}
      />
...
function Toast({ toast }) {
  ...
  return (
    <div
      className={`fixed bottom-6 left-1/2 -translate-x-1/2 z-500 flex items-center gap-3 px-5 py-3 rounded-2xl text-white shadow-2xl text-sm font-medium pointer-events-none sh-fi ${bg}`}
    >
      <I size={16} />
      {toast.msg}
    </div>
...
function ShiftBar({ start, end, color }) {
  ...
  return (
    <div
      className="relative w-full h-1.5 bg-gray-100 rounded-full overflow-visible mt-2"
      title={`${to12(start)} – ${to12(end)}`}
    >
      <div
        className={`absolute top-0 h-1.5 rounded-full ${color.bar}`}
        style={{
...
function ShiftCard({ shift, onView, onEdit, onDelete, idx }) {
  ...
  const overnight =
    toMinutes(toHHMM(shift.shift_end_timing)) <
    toMinutes(toHHMM(shift.shift_start_timing));

  return (
    <div
      className={`sh-su bg-white rounded-2xl shadow-sm border border-gray-100 p-5 hover:shadow-md transition-shadow flex flex-col gap-4`}
    ...
    >
      ...
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-center gap-3 min-w-0">
          <span
            className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${color.bg} ${color.text} border border-opacity-30`}
          >
            <Icon size={18} />
          </span>
      ...
      {/* Timeline bar */}
      <ShiftBar
        start={shift.shift_start_timing}
        end={shift.shift_end_timing}
        color={color}
      />
      ...
      <div className="grid grid-cols-3 gap-2">
        {[
          ...
          {
            label: "Late Buffer",
            value: shift.shift_late_on ? to12(shift.shift_late_on) : "None",
          },
          ...
          >
            <p className="text-[9px] uppercase tracking-wider font-semibold text-gray-400 mb-0.5">
              {label}
            </p>
      ...
      <div className="flex flex-wrap gap-1.5">
        ...
        <span
          className={`inline-flex items-center gap-1 text-[11px] font-medium px-2 py-0.5 rounded-full border ${color.badge}`}
        >
...
function ShiftSidebar({ shift, onClose, onEdit, onDelete }) {
  ...
  return (
    <>
      <div
        className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm sh-fi"
        onClick={onClose}
      />
      <aside className="fixed right-0 top-0 bottom-0 z-51 w-full sm:w-105 bg-white shadow-2xl flex flex-col sh-sir">
        ...
        <div className="flex items-start gap-4 px-6 py-5 border-b border-gray-100">
          <span
            className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 ${color.bg} ${color.text}`}
          >
            <Icon size={22} />
          </span>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {[
                ...
                {
                  label: "Late Buffer",
                  value: shift.shift_late_on
                    ? to12(shift.shift_late_on)
                    : "No buffer",
                },
                {
                  label: "Saturday",
                  value: shift.saturday_on ? "Included" : "Off",
                },
                {
                  label: "Remote Work",
                  value: shift.allow_remote ? "Allowed" : "On-site only",
                },
              ].map(({ label, value }) => (
          ...
          <div>
            ...
            <div className="space-y-2">
              {[
                {
                  label: "Saturday included in work week",
                  on: shift.saturday_on,
                },
                { label: "Remote / WFH allowed", on: shift.allow_remote },
              ].map(({ label, on }) => (
                <div
                  key={label}
                  className={`flex items-center justify-between px-3 py-2.5 rounded-xl border ${on ? "border-blue-100 bg-blue-50" : "border-gray-100 bg-gray-50"}`}
                >
                  <span className="text-sm text-gray-700">{label}</span>
                  <span
                    className={`text-xs font-semibold px-2 py-0.5 rounded-full ${on ? "bg-blue-100 text-blue-700" : "bg-gray-200 text-gray-500"}`}
                  >
                    {on ? "Yes" : "No"}
                  </span>
                </div>
...
const EMPTY = {
  name: "",
  shift_start_timing: "08:00",
  shift_end_timing: "16:00",
  shift_late_on: "",
  saturday_on: false,
  allow_remote: false,
};
...
function ShiftModal({ shift, onClose, onSaved, showToast }) {
  ...
  const validate = () => {
    ...
    if (!form.shift_start_timing)
      e.shift_start_timing = "Start time is required.";
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg flex flex-col max-h-[92vh] overflow-hidden">
        ...
        <div className="flex items-center gap-3 px-6 py-4 border-b border-gray-100">
          ...
          <div>
            ...
            <p className="text-[11px] text-gray-400">
              {isEdit ? `Editing: ${shift.name}` : "Define a new work shift"}
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          ...
          {form.shift_start_timing && form.shift_end_timing && (
            <div className="flex items-center gap-2.5 bg-blue-50 border border-blue-100 rounded-xl px-4 py-2.5">
              ...
              <p className="text-sm text-blue-800">
                ...
                {toMinutes(form.shift_end_timing) <
                  toMinutes(form.shift_start_timing) && (
          ...
          <div>
            <label className="block text-[11px] uppercase tracking-widest font-semibold text-gray-400 mb-1.5">
              ...
              <span className="normal-case font-normal text-gray-400">
                (optional — when to mark as late)
              </span>
            </label>
            <input
              type="time"
              value={form.shift_late_on}
          ...
          <div className="space-y-3">
            ...
            {[
              {
                key: "saturday_on",
                label: "Include Saturday in work week",
                sub: "Employees will be expected on Saturdays",
                icon: Coffee,
              },
              {
                key: "allow_remote",
                label: "Allow remote / work from home",
                sub: "Employees can work from any location",
                icon: Wifi,
              },
              ...
              >
                <div className="flex items-center gap-3">
                  <Icon
                    size={16}
                    className={form[key] ? "text-blue-600" : "text-gray-400"}
                  />
                  <div>
              ...
              <div className="bg-gray-50 rounded-xl p-3 border border-gray-100">
                ...
                {form.shift_late_on && (
                  <div className="relative w-full h-1 mt-0.5">
                    {(() => {
                      const pos =
                        (toMinutes(form.shift_late_on) / (24 * 60)) * 100;
                      return (
                        <div
                          className="absolute top-0 w-0.5 h-1 bg-red-400 rounded-full"
                          style={{ left: `${pos}%` }}
                          title="Late marker"
                        />
                      );
        ...
        <div className="px-6 py-4 border-t border-gray-100 flex gap-3">
          ...
          >
            {saving ? (
              <>
                <Loader2 size={14} className="sh-sp" />{" "}
      ...
      <div className="bg-white rounded-2xl p-6 w-95 max-w-full shadow-2xl">
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-2.5 flex items-start gap-2 mb-5">
          ...
          <p className="text-xs text-amber-700">
            Employees currently assigned this shift may be affected. This action
            cannot be undone.
          </p>
        </div>
        <div className="flex gap-3">
          ...
          >
            {deleting ? (
              <>
                <Loader2 size={14} className="sh-sp" /> Deleting…
              </>
            ) : (
              "Yes, Delete"
            )}
...
export default function ShiftsPage() {
  ...
  const fetchShifts = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(API.ListShifts, { headers });
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      const res = await fetch(API.DeleteShift(deleteTarget.id), {
        method: "DELETE",
      });
      if (!res.ok) throw new Error(`Error ${res.status}`);
      ...
      <header className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Create and manage work shifts · Super Admin
          </p>
        </div>
        <button
