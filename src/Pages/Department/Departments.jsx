...
import {
  X, Search, ChevronLeft, ChevronRight, Users, Building2,
  Edit3, Trash2, Eye, PlusCircle, Send, RefreshCw,
  Mail, Phone, MapPin, Calendar, Award, UserCog, UserCheck,
  ArrowLeft, TrendingUp, CheckCircle2, XCircle, Loader2,
  AlertTriangle, Info, ChevronDown, ChevronRight as CRight,
} from "lucide-react";
...
`;
if (typeof document !== "undefined" && !document.getElementById("__dp_styles__")) {
  const el = Object.assign(document.createElement("style"), { id: "__dp_styles__", textContent: _STYLES });
  document.head.appendChild(el);
}

/* ─── Dept visual metadata (client-side decoration only) ─── */
const DEPT_PALETTE = [
  { color: "bg-indigo-50 text-indigo-700", border: "border-indigo-200", accent: "#6366f1", icon: "⚙️" },
  { color: "bg-violet-50 text-violet-700", border: "border-violet-200", accent: "#7c3aed", icon: "👥" },
  { color: "bg-orange-50 text-orange-700", border: "border-orange-200", accent: "#ea580c", icon: "📈" },
  { color: "bg-green-50 text-green-700",   border: "border-green-200",  accent: "#16a34a", icon: "💰" },
  { color: "bg-pink-50 text-pink-700",     border: "border-pink-200",   accent: "#db2777", icon: "🎨" },
  { color: "bg-cyan-50 text-cyan-700",     border: "border-cyan-200",   accent: "#0891b2", icon: "🔧" },
  { color: "bg-amber-50 text-amber-700",   border: "border-amber-200",  accent: "#d97706", icon: "📦" },
  { color: "bg-rose-50 text-rose-700",     border: "border-rose-200",   accent: "#e11d48", icon: "🏢" },
];
/* Deterministic palette per dept id */
function dPalette(id) { return DEPT_PALETTE[(id || 0) % DEPT_PALETTE.length]; }
...
/* ─── Helpers ─── */
function empName(e) { return e?.name || e?.f_name +" "+ e?.l_name || `Employee #${e?.id}`; }
function empType(e) { return e?.emp_type || e?.empType || e?.type || "Full-time"; }
function empStatus(e) { return e?.status || "Active"; }
function empDeptId(e) { return e?.department_id || e?.department?.id || null; }
...
function Toast({ toast }) {
  ...
  const { bg, I } = cfg;
  return (
    <div className={`fixed bottom-6 left-1/2 -translate-x-1/2 z-[500] flex items-center gap-3 px-5 py-3 rounded-2xl text-white shadow-2xl text-sm font-medium pointer-events-none dp-fi ${bg}`}>
      <I size={16} /> {toast.msg}
    </div>
  );
}
...
function EmpSidebar({ employee, onClose }) {
  ...
  return (
    <>
      <div className="fixed inset-0 z-[200] bg-black/40 backdrop-blur-sm dp-fi" onClick={onClose} />
      <aside className="fixed right-0 top-0 bottom-0 z-[201] w-full sm:w-[360px] bg-white shadow-2xl flex flex-col dp-sir">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
          <div className="flex items-center gap-4 bg-gray-50 rounded-xl p-4">
            <img src={`https://i.pravatar.cc/150?u=${employee.id}`} alt={empName(employee)}
              className="w-16 h-16 rounded-full object-cover ring-2 ring-white shadow shrink-0" />
            <div className="min-w-0">
              <p className="font-semibold text-gray-900 text-base">{empName(employee)}</p>
              <p className="text-xs text-gray-500 mt-0.5">{employee.position}</p>
              <div className="flex flex-wrap gap-1.5 mt-2">
                <span className={`text-[11px] font-medium px-2 py-0.5 rounded-lg border ${EMP_TYPE_COLOR[type] || "bg-gray-100 text-gray-600 border-gray-200"}`}>{type}</span>
                <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full flex items-center gap-1 ${stCfg.badge}`}>
                  <span className={`w-1.5 h-1.5 rounded-full ${stCfg.dot}`} /> {empStatus(employee)}
                </span>
              </div>
            </div>
          </div>
          {[
            ...
            { Icon: Calendar, label: "Joined", value: employee.join_date || employee.joinDate },
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="bg-gray-50 rounded-xl p-3">
              <p className="text-[10px] uppercase tracking-wider font-semibold text-gray-400 mb-1">Employee ID</p>
              <p className="text-sm font-medium text-gray-800">#{employee.id}</p>
            </div>
            {employee.salary && (
              <div className="bg-gray-50 rounded-xl p-3">
                <p className="text-[10px] uppercase tracking-wider font-semibold text-gray-400 mb-1">Salary</p>
                <p className="text-sm font-medium text-gray-800">PKR {Number(employee.salary).toLocaleString()}</p>
              </div>
            )}
          </div>
        </div>
        <div className="px-6 py-4 border-t border-gray-100">
          <button onClick={onClose} className="w-full py-2.5 border border-gray-200 text-gray-600 hover:bg-gray-50 text-sm font-medium rounded-xl transition">Close</button>
        </div>
      </aside>
    </>
  );
...
function DeptFormModal({ dept, onClose, onSaved, showToast }) {
  ...
  const handleSubmit = async () => {
    ...
    setSaving(true);
    const payload = { department: name.trim(), extra_data: dept?.extra_data || null };
    try {
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm dp-fi px-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
        <div className="flex items-center gap-3 px-6 py-4 border-b border-gray-100">
          ...
          <div>
            <p className="font-semibold text-gray-900 text-sm">{isEdit ? "Edit Department" : "New Department"}</p>
            <p className="text-[11px] text-gray-400">{isEdit ? `Editing: ${dept.department}` : "Add a new department"}</p>
          </div>
        ...
        <div className="px-6 py-5">
          <label className="block text-[11px] uppercase tracking-widest font-semibold text-gray-400 mb-1.5">Department Name *</label>
        ...
        <div className="px-6 pb-5 flex gap-3">
          <button onClick={handleSubmit} disabled={saving}
            ...
            {saving ? <><Loader2 size={14} className="dp-sp" />{isEdit ? "Saving…" : "Creating…"}</> : <><Send size={14} />{isEdit ? "Save" : "Create"}</>}
          </button>
          <button onClick={onClose} disabled={saving}
            className="py-2.5 px-5 border border-gray-200 text-gray-600 hover:bg-gray-50 disabled:opacity-40 text-sm font-medium rounded-xl transition">Cancel</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/50 backdrop-blur-sm dp-fi px-4">
      <div className="bg-white rounded-2xl p-6 w-[380px] max-w-full shadow-2xl">
        ...
        <h2 className="text-base font-semibold text-gray-900 text-center mb-1">Delete Department?</h2>
        <p className="text-sm font-bold text-gray-800 text-center mb-4">"{dept.department}"</p>
        <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-2.5 flex items-start gap-2 mb-5">
          <AlertTriangle size={14} className="text-amber-600 mt-0.5 shrink-0" />
          <p className="text-xs text-amber-700">Employees assigned to this department may be affected. This cannot be undone.</p>
        </div>
        <div className="flex gap-3">
          ...
          <button onClick={onConfirm} disabled={deleting}
            ...
            {deleting ? <><Loader2 size={14} className="dp-sp" />Deleting…</> : "Yes, Delete"}
          </button>
        </div>
      </div>
    </div>
...
function AssignHeadModal({ dept, allEmployees, onClose, onAssign, assigning }) {
  ...
  const candidates = useMemo(() => {
    const inDept = allEmployees.filter(e => {
      const dId = empDeptId(e);
      if (dId) return dId === dept.id;
      const dName = typeof e.department === "string" ? e.department : e.department?.department;
      return dName?.toLowerCase() === dept.department?.toLowerCase();
    }).filter(e => empStatus(e) === "Active");

    if (!search) return inDept;
    const q = search.toLowerCase();
    return inDept.filter(e => empName(e).toLowerCase().includes(q) || e.position?.toLowerCase().includes(q));
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm dp-fi px-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md flex flex-col max-h-[88vh] overflow-hidden">
        ...
        {/* Header */}
        <div className="flex items-center gap-3 px-6 py-4 border-b border-gray-100">
          <div className={`w-9 h-9 rounded-xl flex items-center justify-center text-lg shrink-0 ${dPalette(dept.id).color}`}>
            {dPalette(dept.id).icon}
          </div>
          <div>
            <p className="font-semibold text-gray-900 text-sm">Assign Department Head</p>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-3 space-y-2">
          ...
          <div onClick={() => setSelected(null)}
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-gray-700">No Head Assigned</p>
              <p className="text-xs text-gray-400">Remove current assignment</p>
            </div>
            {selected === null && <CheckCircle2 size={16} className="text-red-500 shrink-0" />}
          </div>

          {candidates.length === 0 && search === "" && (
            <div className="text-center py-8">
              <Users size={24} className="mx-auto mb-2 text-gray-300" />
              <p className="text-sm text-gray-400">No active employees in {dept.department}</p>
            </div>
          )}
          {candidates.length === 0 && search !== "" && (
            <div className="text-center py-6 text-sm text-gray-400">No matches found.</div>
          )}
          ...
          {candidates.map(emp => {
            ...
            return (
              <div key={emp.id} onClick={() => setSelected(emp.id)}
                ...
                <img src={`https://i.pravatar.cc/150?u=${emp.id}`} alt={empName(emp)}
                  className="w-9 h-9 rounded-full object-cover border-2 border-white shadow-sm shrink-0" />
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-gray-900">{empName(emp)}</p>
                  <p className="text-xs text-gray-400 truncate">{emp.position}</p>
                </div>
                <span className={`text-[11px] font-medium px-2 py-0.5 rounded-lg border shrink-0 ${EMP_TYPE_COLOR[type] || "bg-gray-100 text-gray-600 border-gray-200"}`}>{type}</span>
                {isSel && <CheckCircle2 size={16} className="text-blue-500 shrink-0" />}
        ...
        <div className="px-6 py-4 border-t border-gray-100 flex gap-3">
          <button onClick={() => onAssign(dept.id, selected)} disabled={assigning}
            className="flex-1 py-2.5 bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 text-white text-sm font-semibold rounded-xl transition flex items-center justify-center gap-2">
            {assigning ? <><Loader2 size={14} className="dp-sp" />Saving…</> : <><Send size={14} />Confirm Assignment</>}
          </button>
...
function DepartmentDetail({ dept, allEmployees, allDepts, onBack, onAssignHead, onEditDept }) {
  ...
  const head = dept.head || allEmployees.find(e => e.id === dept.head_id) || null;
  ...
  const stats = useMemo(() => ({
    ...
    avgSalary: deptEmps.length
      ? Math.round(deptEmps.reduce((s, e) => s + (e.salary || 0), 0) / deptEmps.length / 1000)
      : 0,
  }), [deptEmps]);

  const EMP_TYPES   = ["Full-time", "Lead", "Intern", "Contract"];
  const EMP_STATUSES = ["Active", "On Leave", "Probation", "Terminated"];
  const typeBreakdown = EMP_TYPES.map(t => ({ type: t, count: deptEmps.filter(e => empType(e) === t).length })).filter(x => x.count > 0);

  const filtered = deptEmps.filter(e =>
    (!search || empName(e).toLowerCase().includes(search.toLowerCase()) || e.position?.toLowerCase().includes(search.toLowerCase())) &&
  ...
  const paginated  = filtered.slice((page - 1) * DETAIL_PAGE_SIZE, page * DETAIL_PAGE_SIZE);
    ...
    <div className="dp-su">
      ...
      <div className="mb-6 flex flex-wrap items-center gap-3">
        ...
        <div className="flex items-center gap-3">
          <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-xl ${palette.color}`}>{palette.icon}</div>
          <div>
            <h1 className="text-2xl font-semibold text-gray-900">{dept.department}</h1>
            <p className="text-xs text-gray-400">ID #{dept.id} · Created {dept.created_at ? new Date(dept.created_at).toLocaleDateString() : "—"}</p>
          </div>
        </div>
        <div className="ml-auto flex gap-2">
          <button onClick={() => onEditDept(dept)}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <Widget icon={Users}       title="Total Employees" value={stats.total}      sub="In department"   color="border-blue-200 bg-blue-50 text-blue-700"      />
        <Widget icon={CheckCircle2}title="Active"          value={stats.active}     sub="Currently working" color="border-green-200 bg-green-50 text-green-700"  />
        <Widget icon={Calendar}    title="On Leave"        value={stats.onLeave}    sub="Away"            color="border-yellow-200 bg-yellow-50 text-yellow-700" />
        <Widget icon={TrendingUp}  title="Avg. Salary"     value={`${stats.avgSalary}K`} sub="PKR / month" color="border-violet-200 bg-violet-50 text-violet-700" />
      </div>
        ...
        <Card title="Department Head">
          {head ? (
            <div className="flex flex-col items-center text-center gap-3 py-2">
              <div className="relative">
                <img src={`https://i.pravatar.cc/150?u=${head.id}`} alt={empName(head)}
              ...
              <div>
                <p className="font-bold text-gray-900 text-base">{empName(head)}</p>
                <p className="text-xs text-gray-500 mt-0.5">{head.position}</p>
                <div className="flex justify-center gap-1.5 mt-2">
                  <span className={`text-[11px] font-medium px-2 py-0.5 rounded-lg border ${EMP_TYPE_COLOR[empType(head)] || "bg-gray-100 text-gray-600 border-gray-200"}`}>{empType(head)}</span>
                  <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full flex items-center gap-1 ${EMP_STATUS_CFG[empStatus(head)]?.badge}`}>
                    <span className={`w-1.5 h-1.5 rounded-full ${EMP_STATUS_CFG[empStatus(head)]?.dot}`} /> {empStatus(head)}
                  </span>
                </div>
              </div>
              <div className="w-full space-y-1.5">
                {[
                  ...
                  { I: Calendar, v: head.join_date || head.joinDate ? `Joined ${head.join_date || head.joinDate}` : null },
              ...
              <button onClick={onAssignHead} className="text-xs text-blue-600 hover:underline font-medium">Change Head</button>
            </div>
          ) : (
            <div className="flex flex-col items-center gap-3 py-8">
              ...
              <button onClick={onAssignHead} className="text-sm text-blue-600 font-medium hover:underline">+ Assign a head</button>
            </div>
          )}
        </Card>
        ...
        <Card title="Department Info">
          <div>
            {[
              ...
              { label: "Head ID",     value: dept.head_id ? `#${dept.head_id}` : "Not assigned" },
          ...
          <div className="space-y-3">
              ...
              return (
                <div key={type}>
                  <div className="flex items-center justify-between mb-1">
                    <span className={`text-[11px] font-medium px-2 py-0.5 rounded-lg border ${EMP_TYPE_COLOR[type] || "bg-gray-100 text-gray-600 border-gray-200"}`}>{type}</span>
                    <span className="text-xs font-semibold text-gray-700">{count} <span className="text-gray-400 font-normal">({pct}%)</span></span>
                  </div>
                  <div className="w-full bg-gray-100 rounded-full h-2 overflow-hidden">
                    <div className="h-2 rounded-full transition-all" style={{ width: `${pct}%`, background: palette.accent }} />
                  </div>
                </div>
              );
            })}
            ...
            {typeBreakdown.length > 0 && (
              <div className="pt-2 border-t border-gray-100">
                <p className="text-[10px] uppercase tracking-widest font-semibold text-gray-400 mb-2">Status breakdown</p>
                <div className="flex flex-wrap gap-3">
                  {[
                    { label: "Active",    val: stats.active,    dot: "bg-green-500"  },
                    { label: "On Leave",  val: stats.onLeave,   dot: "bg-yellow-400" },
                    { label: "Probation", val: stats.probation, dot: "bg-blue-500"   },
                  ].map(({ label, val, dot }) => (
                    <div key={label} className="flex items-center gap-1.5 text-xs text-gray-600">
                      <span className={`w-2 h-2 rounded-full ${dot}`} /> {label}: <span className="font-semibold">{val}</span>
                    </div>
                  ))}
                </div>
              </div>
        ...
        action={
          <div className="flex flex-wrap items-center gap-2">
            <div className="relative">
              ...
              <input value={search} onChange={e => sf(() => setSearch(e.target.value))} placeholder="Search…"
            ...
            <select value={filterType} onChange={e => sf(() => setFilterType(e.target.value))}
              ...
              {["Full-time","Lead","Intern","Contract"].map(t => <option key={t}>{t}</option>)}
            </select>
            <select value={filterSt} onChange={e => sf(() => setFilterSt(e.target.value))}
              className="text-xs bg-gray-50 border border-gray-200 rounded-lg px-2 py-1.5 outline-none cursor-pointer text-gray-700">
              <option value="All">All Statuses</option>
              {["Active","On Leave","Probation","Terminated"].map(s => <option key={s}>{s}</option>)}
            </select>
          </div>
        }
      >
          ...
          <table className="min-w-full text-sm border-collapse">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-100">
                {["Employee", "Position", "Type", "Status", "Joined", ""].map((h, i) => (
                  <th key={i} className={`py-3 px-4 text-[11px] font-semibold uppercase tracking-wider text-gray-400 whitespace-nowrap ${i <= 1 ? "text-left" : "text-center"}`}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
                  ...
                  <tr key={emp.id}
                    ...
                    <td className="py-3 px-4 cursor-pointer" onClick={() => setViewEmp(emp)}>
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          <p className="text-sm font-medium text-blue-600 hover:underline whitespace-nowrap">{empName(emp)}</p>
                          <p className="text-xs text-gray-400">#{emp.id}{isHead && <span className="ml-1 text-amber-600 font-semibold">· Head</span>}</p>
                        </div>
                      </div>
                    </td>
                    <td className="py-3 px-4 text-sm text-gray-700 whitespace-nowrap">{emp.position || "—"}</td>
                    <td className="py-3 px-4 text-center">
                      <span className={`text-xs font-medium px-2.5 py-1 rounded-lg border ${EMP_TYPE_COLOR[type] || "bg-gray-100 text-gray-600 border-gray-200"}`}>{type}</span>
                    </td>
                    <td className="py-3 px-4 text-center">
                      <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold ${stCfg.badge}`}>
                        <span className={`w-1.5 h-1.5 rounded-full ${stCfg.dot}`} /> {empStatus(emp)}
                      </span>
                    </td>
        ...
        {totalPages > 1 && (
          <div className="flex justify-between items-center mt-4 pt-3 border-t border-gray-100 text-sm">
            ...
            <span className="text-xs text-gray-500">{(page - 1) * DETAIL_PAGE_SIZE + 1}–{Math.min(page * DETAIL_PAGE_SIZE, filtered.length)} of {filtered.length}</span>
      ...
      {viewEmp && <EmpSidebar employee={viewEmp} onClose={() => setViewEmp(null)} />}
    </div>
  );
}
...
function DeptCard({ dept, allEmployees, onClick, onEdit, onDelete, idx }) {
  ...
  return (
    <div className="dp-su bg-white rounded-2xl shadow-sm border-2 border-transparent hover:border-blue-100 hover:shadow-md transition group overflow-hidden"
      ...
      <div className="p-5">
        ...
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center gap-3 cursor-pointer min-w-0" onClick={onClick}>
            <div className={`w-11 h-11 rounded-xl flex items-center justify-center text-2xl shrink-0 ${palette.color}`}>{palette.icon}</div>
            <div className="min-w-0">
              <p className="font-bold text-gray-900 text-base leading-tight truncate">{dept.department}</p>
              <p className="text-xs text-gray-400 mt-0.5">{deptEmps.length} employees · #{dept.id}</p>
            </div>
          </div>
          {/* Actions */}
        ...
        {/* Head card */}
        <div className={`rounded-xl p-3 mb-4 ${head ? "bg-gray-50" : "bg-gray-50 border border-dashed border-gray-200"}`}>
          {head ? (
            <div className="flex items-center gap-3">
              <div className="relative shrink-0">
                <img src={`https://i.pravatar.cc/150?u=${head.id}`} alt={empName(head)}
              ...
              <div className="min-w-0">
                <p className="text-xs font-semibold text-gray-800 truncate">{empName(head)}</p>
                <p className="text-[11px] text-gray-400 truncate">{head.position}</p>
              </div>
              <span className="ml-auto text-[10px] font-semibold text-amber-700 bg-amber-50 px-1.5 py-0.5 rounded-md border border-amber-200 shrink-0">Head</span>
            </div>
          ) : (
        ...
        <div className="grid grid-cols-3 gap-2 mb-3">
          {[
            ...
            { label: "Interns", val: interns, cls: "text-amber-700 bg-amber-50"   },
          ].map(({ label, val, cls }) => (
        ...
        <div className="pt-3 border-t border-gray-100 flex items-center justify-between">
          ...
          <button onClick={onClick} className="text-xs font-medium text-blue-600 group-hover:underline flex items-center gap-1">
            Open <CRight size={12} />
          </button>
        </div>
      </div>
...
export default function DepartmentPage() {
  ...
  const fetchDepts = useCallback(async () => {
    setDeptsLoading(true); setDeptsErr(null);
    try {
  ...
  const fetchEmployees = useCallback(async () => {
    setEmpsLoading(true); setEmpsErr(null);
    try {
  ...
  const handleAssignHead = async (deptId, empId) => {
    ...
    try {
      ...
      setDepts(prev => prev.map(d => d.id === deptId ? { ...d, ...updated, head_id: empId } : d));
      showToast(empId ? "Department head assigned." : "Head removed.", "success");
      setAssignDept(null);
    } catch (err) {
  ...
  const handleSaved = (saved, isEdit) => {
    setDepts(prev => isEdit ? prev.map(d => d.id === saved.id ? { ...d, ...saved } : d) : [saved, ...prev]);
  };
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      const res = await fetch(API.deleteDepartment(deleteDept.id), { method: "DELETE" });
      if (!res.ok) throw new Error(`Error ${res.status}`);
  ...
  /* ── Derived ── */
  const activeDept = activeDeptId ? depts.find(d => d.id === activeDeptId) : null;
    ...
    <div className="min-h-screen bg-gray-50 px-6 py-10 text-gray-900">
      ...
      {activeDept ? (
        <DepartmentDetail
          ...
          allDepts={depts}
          onBack={() => { setActiveDeptId(null); setSearch(""); }}
        ...
        <>
          ...
          <header className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-6">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">Manage teams, heads & organisation structure</p>
            </div>
          ...
          {/* Widgets */}
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            <Widget icon={Building2} title="Departments"    value={loading ? "…" : stats.total}    sub="Total"             color="border-blue-200 bg-blue-50 text-blue-700"      />
            <Widget icon={Users}     title="Employees"      value={loading ? "…" : stats.empTotal} sub="Across all depts"  color="border-green-200 bg-green-50 text-green-700"   />
            <Widget icon={UserCog}   title="Heads Assigned" value={loading ? "…" : stats.withHead} sub="Departments led"   color="border-violet-200 bg-violet-50 text-violet-700" />
            <Widget icon={UserCheck} title="No Head"        value={loading ? "…" : stats.noHead}   sub="Needs assignment"  color="border-amber-200 bg-amber-50 text-amber-700"   />
          </div>

          {/* Errors */}
          {deptsErr && <div className="mb-4"><ErrBanner msg={deptsErr} onRetry={fetchDepts} /></div>}
          {empsErr  && <div className="mb-4"><ErrBanner msg={empsErr}  onRetry={fetchEmployees} /></div>}
          ...
          <div className="flex items-center gap-3 mb-5">
            ...
            <button onClick={() => { fetchDepts(); fetchEmployees(); }} disabled={loading}
              ...
              {loading ? <Loader2 size={14} className="dp-sp" /> : <RefreshCw size={14} />} Refresh
            </button>
            <span className="text-xs text-gray-400 ml-auto">{filteredDepts.length} dept{filteredDepts.length !== 1 ? "s" : ""}</span>
          </div>
          ...
          {loading ? (
            ...
            <div className="bg-white rounded-2xl p-16 text-center shadow-sm">
              ...
              <p className="text-sm text-gray-400 mb-3">{search ? "No departments match your search." : "No departments found."}</p>
              {!search && (
                <button onClick={() => setShowCreate(true)} className="text-sm text-blue-600 font-medium hover:underline">+ Create first department</button>
              )}
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {filteredDepts.map((dept, i) => (
                <DeptCard key={dept.id} dept={dept} allEmployees={allEmployees} idx={i}
                  onClick={() => { setActiveDeptId(dept.id); window.scrollTo(0, 0); }}
      ...
      {/* ── Overlays ── */}
      {showCreate && (
        <DeptFormModal onClose={() => setShowCreate(false)} onSaved={handleSaved} showToast={showToast} />
      )}
      {editDept && (
        <DeptFormModal dept={editDept} onClose={() => setEditDept(null)} onSaved={handleSaved} showToast={showToast} />
      )}
      {assignDept && (
        <AssignHeadModal
          dept={assignDept}
      ...
      {deleteDept && (
        <DeleteConfirm dept={deleteDept} deleting={deleting}
          onConfirm={handleDelete} onCancel={() => setDeleteDept(null)} />
      )}
