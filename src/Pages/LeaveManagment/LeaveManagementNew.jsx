...
const API = {
  GetAllApplications: (params = "") => `${BASE}/leaves/applications${params}`,
  GetPendingApplications: (params = "") =>
    `${BASE}/leaves/applications/pending${params}`,
  GetApplicationsByEmployee: (id) =>
    `${BASE}/leaves/applications/employee/${id}`,
  GetApplication: (id) => `${BASE}/leaves/applications/${id}`,
  ApproveApplication: (id) => `${BASE}/leaves/applications/${id}/approve`,
  RejectApplication: (id) => `${BASE}/leaves/applications/${id}/reject`,
  GetAllCycles: () => `${BASE}/leaves/cycles`,
  GetActiveCycle: () => `${BASE}/leaves/cycles/active`,
  CreateCycle: () => `${BASE}/leaves/cycles`,
  UpdateCycle: (id) => `${BASE}/leaves/cycles/${id}`,
  DeleteCycle: (id) => `${BASE}/leaves/cycles/${id}`,
  GetAllTypes: () => `${BASE}/leaves/types`,
  GetTypesByCycle: (id) => `${BASE}/leaves/types/cycle/${id}`,
  CreateType: () => `${BASE}/leaves/types`,
  UpdateType: (id) => `${BASE}/leaves/types/${id}`,
  DeleteType: (id) => `${BASE}/leaves/types/${id}`,
  GetDashboardStats: () => `${BASE}/leaves/dashboard/stats`,
  GetEmployeeSummary: (id) => `${BASE}/leaves/summary/employee/${id}`,
  GetManagementAttachments: (id) =>
    `${BASE}/leaves/management/applications/${id}/attachments`,
  DownloadManagementAttachment: (id) =>
    `${BASE}/leaves/management/attachments/${id}/download`,
};
  ...
  return (
    <div className="fixed top-5 right-5 z-[200] flex flex-col gap-2 pointer-events-none">
      {toasts.map((t) => (
        <div
          key={t.id}
          className={`flex items-center gap-2.5 px-4 py-3 rounded-2xl shadow-lg text-sm font-medium text-white ${t.type === "success" ? "bg-emerald-500" : "bg-red-500"}`}
        >
          {t.type === "success" ? (
...
function StatCard({ label, value, icon: Icon, color }) {
  ...
  return (
    <div
      className={`rounded-2xl p-5 border border-gray-100 ${colors.bg} bg-white`}
    >
      <div
        className={`w-10 h-10 rounded-xl flex items-center justify-center mb-3 ${colors.icon}`}
      >
        <Icon size={18} />
      </div>
              ...
              <div className="space-y-1.5">
                {atts.map((a) => (
                  ...
                  >
                    ...
                    <a
                      href={API.DownloadManagementAttachment(a.id)}
                      target="_blank"
                      rel="noreferrer"
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            ...
            {[
              ...
              ["total_per_cycle", "Total/Cycle (0=unlimited)", 0],
            ].map(([k, l, mn]) => (
              <div key={k}>
                <label className="text-xs uppercase tracking-widest font-bold text-gray-400 mb-1.5 block">
                  {l}
...
export default function LeaveManagement() {
  ...
  const filteredApps = apps.filter((a) => {
    ...
    const matchSearch =
      !search ||
      String(a.employee_id).includes(search) ||
    ...
    <section className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <button
            onClick={loadAll}
            className="w-9 h-9 flex items-center justify-center rounded-xl border border-gray-200 text-gray-500 hover:bg-white transition"
          >
            <RefreshCw size={15} />
          </button>
        </div>
                      ...
                      <thead>
                        <tr className="border-b border-gray-100">
                          {[
                            ...
                            <th
                              key={h}
                              className="px-5 py-3 text-left text-xs font-bold text-gray-400 uppercase tracking-widest whitespace-nowrap"
                            >
                              {h}
                            </th>
                          ))}
            ...
            {tab === "dashboard" && stats && (
              <div className="space-y-6">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  ...
                  <StatCard
                    label="Approval Rate"
                    value={`${stats.approved_vs_rejected?.ratio ?? 0}%`}
                    icon={TrendingUp}
                    color="emerald"
                  />
                  <StatCard
                    label="Rejected"
                    value={stats.approved_vs_rejected?.rejected ?? 0}
                    icon={XCircle}
                    color="red"
                  />
                </div>
                  ...
                  <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                    ...
                    <div className="flex items-end gap-2 h-32">
                      {stats.monthly_trend.map((m) => {
                        const max = Math.max(
                          ...stats.monthly_trend.map((x) => x.total_leaves),
                          1,
                        );
                  ...
                  <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                    ...
                    <div className="space-y-3">
                      {stats.department_usage.map((d) => {
                        const max = Math.max(
                          ...stats.department_usage.map((x) => x.total_leaves),
                          1,
                        );
                        return (
                          <div
                    ...
                    {cycles.map((c) => (
                      ...
                      >
                        <div className="flex items-center gap-4">
                          <div
                            className={`w-2 h-10 rounded-full ${c.is_active ? "bg-emerald-400" : "bg-gray-200"}`}
                          />
                          <div>
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            onClick={() => setCycleModal(c)}
                            className="px-3 py-1.5 border border-gray-200 text-gray-600 text-xs font-semibold rounded-lg hover:bg-gray-100 transition"
                          >
                            Edit
                          </button>
                          <button
                        ...
                        {types.map((t) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <td className="px-5 py-4">
                                <span
                                  className={`text-xs font-semibold px-2 py-0.5 rounded-full ${t.is_paid ? "bg-emerald-50 text-emerald-700" : "bg-gray-100 text-gray-500"}`}
                                >
                              ...
                              <td className="px-5 py-4 text-center">
                                {t.requires_document ? (
                                  <span className="w-5 h-5 bg-blue-100 text-blue-600 rounded-full flex items-center justify-center mx-auto">
                                    <Paperclip size={10} />
                                  </span>
                                ) : (
