 ...
 *
 * ── Auth ────────────────────────────────────────────────────
 *  Reads access_token → JWT → employee DB id
 *  Priority: EPI → employee_id → id → sub
 *
 * ── APIs needed in Apis.js ──────────────────────────────────
 *  MyAttendance        : `${mainOrigin}/attendance/me`
 *  MyAttendanceByMonth : (empId, month) =>
 *                          `${mainOrigin}/attendance/me?employee_id=${empId}&month=${month}`
 *  MyAttendanceToday   : (empId) =>
 *                          `${mainOrigin}/attendance/me/today?employee_id=${empId}`
 *  MyAttendanceSummary : (empId, month) =>
 *                          `${mainOrigin}/attendance/me/summary?employee_id=${empId}&month=${month}`
 *  shiftDetailsById    : (empId) =>
 *                          `${mainOrigin}/shifts/employee/${empId}`
 *
 * ── What the backend returns ─────────────────────────────────
 *  /me/today  → single AttendanceRecord already resolved by backend
 *               (overnight post-midnight is handled server-side)
 *  /me?month  → AttendanceRecord[]  all dated on the IN-punch date
 *  /me/summary→ AttendanceSummary
 *
 *  AttendanceRecord {
 *    id, employee_id,
 *    date     : "YYYY-MM-DD"  ← always the IN-punch (check-in) date
 *    status   : "Present" | "Late" | "Absent" | "Leave"
 *    in_time  : "HH:MM" | null
 *    out_time : "HH:MM" | null   ← may be from next calendar day for overnight
 *    hours    : float | null      ← cross-midnight hours already computed
 *    note     : string | null
 *  }
 */
...
import {
  ...
  X, Search, LogIn, LogOut, Timer, Info,
  AlertCircle, BarChart2, User, Moon, Sun, Sunset,
} from "lucide-react";
...
const PERIOD_META = {
  day:       { label: "Day Shift",       Icon: Sun,    color: "text-amber-500",  bg: "bg-amber-50 border-amber-200"   },
  afternoon: { label: "Afternoon Shift", Icon: Sunset, color: "text-orange-500", bg: "bg-orange-50 border-orange-200" },
  night:     { label: "Night Shift",     Icon: Moon,   color: "text-indigo-500", bg: "bg-indigo-50 border-indigo-200" },
};
  ...
  return (
    <div className="flex items-start gap-3 bg-red-50 border border-red-200 rounded-xl px-4 py-3">
      ...
      <div className="flex-1 min-w-0">
        <p className="text-xs font-semibold text-red-700">Could not load data</p>
...
function NoAuthBanner() {
  return (
    <div className="flex items-start gap-3 bg-amber-50 border border-amber-200 rounded-xl px-4 py-4 mb-5">
      ...
      <div>
        <p className="text-sm font-semibold text-amber-800">Not authenticated</p>
        <p className="text-xs text-amber-700 mt-0.5">No valid access token found. Please log in.</p>
      </div>
    </div>
  );
}
...
function ShiftBanner({ shift, loading }) {
  if (loading) return <div className="h-10 bg-gray-100 animate-pulse rounded-xl mb-5" />;
  ...
  return (
    <div className={`flex items-center gap-3 px-4 py-2.5 rounded-xl border mb-5 ${meta.bg}`}>
      ...
      <div className="flex items-center gap-2 flex-wrap text-xs">
        ...
        {shift.shift_late_on && (
          <><span className="text-gray-300">·</span>
            <span className="text-gray-400">Late after {to12(shift.shift_late_on)}</span></>
        )}
        {shift.total_hours && (
          <><span className="text-gray-300">·</span>
            <span className="text-gray-400">{fmtShiftTotal(shift.total_hours)} shift</span></>
        )}
      </div>
    </div>
  );
  ...
  return (
    <Card className="p-6">
      ...
      <div className="flex items-start justify-between mb-5">
        <div>
          <p className="text-xs uppercase tracking-widest font-semibold text-gray-400 mb-0.5">Today</p>
          ...
          {shift && (
            <p className={`text-xs mt-0.5 font-medium ${periodMeta.color}`}>
              ...
              {overnight && <span className="ml-1 text-indigo-500 font-semibold"> (+1 day)</span>}
            </p>
          )}
        </div>
        {cfg && (
      ...
      {loading && (
        <div className="flex items-center gap-3 py-2">
          ...
          <span className="text-sm text-gray-400">Checking today's record…</span>
        </div>
      )}
      ...
      {!loading && !error && !rec && (
        <div className="flex items-center gap-3 bg-gray-50 rounded-xl px-4 py-3">
          ...
          <div>
            <p className="text-sm text-gray-400">No attendance record found for today.</p>
            {expIn && (
              <p className="text-xs text-gray-400 mt-0.5">
                Expected check-in: <span className="font-semibold text-gray-600">{expIn}</span>
                {overnight && <span className="text-gray-400"> · out next day at {expOut}</span>}
              </p>
            )}
          </div>
        </div>
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              ...
              <div key={label} className={`flex flex-col items-center gap-1.5 rounded-xl py-4 px-2 ${bg}`}>
                ...
                {sub && <span className="text-[10px] text-gray-400 text-center leading-tight">{sub}</span>}
              </div>
            ))}
          </div>
          ...
          <Card key={key} className="p-5 flex flex-col gap-3">
            ...
            <div>
              <div className="flex justify-between text-[11px] text-gray-400 mb-1 font-medium">
                <span>of {total ?? "—"} days</span><span>{pct}%</span>
              </div>
              <div className="w-full bg-gray-100 rounded-full h-1.5 overflow-hidden">
                <div className={`h-1.5 rounded-full transition-all duration-500 ${bar}`}
    ...
    <Card className="p-5">
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          {[["Present", "bg-emerald-400"], ["Late", "bg-yellow-400"], ["Absent", "bg-red-400"], ["Leave", "bg-slate-400"]].map(([l, c]) => (
    ...
    <Card className="p-6">
      ...
      <div className="flex flex-wrap items-center gap-3 mb-4">
        <h3 className="text-sm font-medium text-gray-500 flex-1 min-w-[80px]">
          Records {!loading && <span className="text-gray-400 font-normal">({filtered.length})</span>}
        </h3>
        <select value={month} onChange={e => { onMonthChange(e.target.value); setPage(1); }}
        ...
        <div className="relative">
          ...
          <input value={q} onChange={e => reset(() => setQ(e.target.value))} placeholder="Search…"
      ...
      {pills.length > 0 && (
        <div className="flex flex-wrap gap-2 mb-3">
          {pills.map(p => (
            <span key={p} className="inline-flex items-center gap-1 px-2.5 py-0.5 bg-gray-900 text-white text-[11px] font-medium rounded-full">
              {p}
              <button onClick={() => { if (p === status) reset(() => setStatus("All")); else reset(() => setQ("")); }}
          ...
          <thead>
            <tr className="bg-gray-50 border-b border-gray-100">
              {[
                ...
                <th key={h} className={`py-3 px-4 text-[11px] font-semibold uppercase tracking-wider text-gray-400 whitespace-nowrap ${a}`}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {loading ? (
               ...
               */
              const outCrossedMidnight = overnight &&
              ...
              return (
                <tr key={`${r.date}-${i}`}
                  ...
                  <td className="py-3 px-4 text-center">
                    <span className={`text-xs font-medium px-2 py-0.5 rounded-lg ${isWknd ? "bg-gray-100 text-gray-400" : "bg-blue-50 text-blue-700"}`}>
                      {dName}
                    </span>
                  </td>

                  <td className="py-3 px-4 text-center">
                    <span className={`inline-flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded-full ${cfg.badge || "bg-gray-100 text-gray-600"}`}>
                  ...
                  <td className="py-3 px-4 text-center">
                    <p className="text-sm font-medium text-gray-700 tabular-nums">{to12(r.in_time)}</p>
                    <p className="text-[10px] text-gray-400">{r.date}</p>
                  </td>

                  {/* Check Out — out_time from backend (punch True), may be next day */}
                  <td className="py-3 px-4 text-center">
                    <p className="text-sm font-medium text-gray-700 tabular-nums">{to12(r.out_time)}</p>
                    {outCrossedMidnight
                      ? <p className="text-[10px] text-indigo-500 font-medium">next day</p>
                      : <p className="text-[10px] text-gray-400">{r.date}</p>}
                  </td>

                  {/* Hours — backend computed cross-midnight correctly */}
                  <td className="py-3 px-4 text-center">
                    <span className="text-sm font-semibold text-gray-800 tabular-nums">{fmtHours(r.hours)}</span>
                  </td>

                  <td className="py-3 px-4 text-center">
                    {r.note
                      ? <span title={r.note} className="inline-block max-w-[110px] truncate text-xs text-amber-700 bg-amber-50 border border-amber-100 px-2 py-0.5 rounded-lg">{r.note}</span>
...
function buildFallbackUser(auth) {
 ...
 useEffect(() => {
  ...
  const loadShift = useCallback(async () => {
    ...
    setShiftLoad(true);
    try {
      const url  = typeof API.shiftDetailsById === "function"
    ...
    <div className="min-h-screen bg-gray-50 px-6 py-10">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Your personal attendance record & monthly overview</p>
        </div>
