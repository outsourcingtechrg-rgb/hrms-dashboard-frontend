...
import {
  Ticket, Plus, Search, X, Loader2, ChevronDown, ChevronLeft,
  ChevronRight, Paperclip, Download, Send, Clock, CheckCircle2,
  AlertTriangle, XCircle, RefreshCw, MessageSquare, Tag, Zap,
  Filter, FileText, Eye, Calendar, User, Building2, Hash,
...
if (typeof document !== "undefined" && !document.getElementById("__tk_sty__")) {
  ...
  s.id = "__tk_sty__"; s.textContent = STYLES;
...
const PRIORITY = {
  ...
  MEDIUM:   { label: "Medium",   color: "text-blue-600",   bg: "bg-blue-50",     border: "border-blue-200",  dot: "bg-blue-500"   },
  HIGH:     { label: "High",     color: "text-amber-600",  bg: "bg-amber-50",    border: "border-amber-300", dot: "bg-amber-500"  },
  CRITICAL: { label: "Critical", color: "text-red-600",    bg: "bg-red-50",      border: "border-red-300",   dot: "bg-red-500"    },
};

const STATUS = {
  OPEN:        { label: "Open",        color: "text-blue-600",   bg: "bg-blue-50",     border: "border-blue-200",   icon: Circle       },
  IN_PROGRESS: { label: "In Progress", color: "text-violet-600", bg: "bg-violet-50",   border: "border-violet-200", icon: Zap          },
  RESOLVED:    { label: "Resolved",    color: "text-emerald-600",bg: "bg-emerald-50",  border: "border-emerald-200",icon: CheckCircle2 },
  CLOSED:      { label: "Closed",      color: "text-slate-500",  bg: "bg-slate-100",   border: "border-slate-200",  icon: CheckCheck   },
...
function PriorityBadge({ priority }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold border ${p.color} ${p.bg} ${p.border}`}>
...
function Toast({ toasts }) {
  return (
    <div className="fixed top-5 right-5 z-[500] flex flex-col gap-2 pointer-events-none">
      {toasts.map((t) => (
        <div key={t.id} className={`flex items-center gap-2.5 px-4 py-3 rounded-2xl shadow-xl text-sm font-semibold text-white tk-pop
...
function CreateModal({ categories, onClose, onCreated, addToast }) {
  const [form, setForm] = useState({ subject: "", category_id: "", priority: "MEDIUM", summary: "" });
  ...
  const submit = async () => {
    ...
    try {
      ...
      for (const file of files) {
        ...
        fd.append("file", file);
        await fetch(TICKET_API.uploadAtt(ticket.id), { method: "POST", headers: tkAuth(), body: fd });
      }
  ...
  const inp = "w-full border rounded-xl px-3.5 py-2.5 text-sm bg-zinc-50 outline-none focus:border-zinc-400 focus:ring-2 focus:ring-zinc-200 transition placeholder-zinc-400";
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-4 max-h-[92vh] flex flex-col overflow-hidden tk-pop">
        ...
        <div className="px-6 py-5 border-b border-zinc-100 flex items-center justify-between bg-zinc-950">
          <div>
            ...
            <p className="text-xs text-zinc-400 mt-0.5">Describe your issue and we'll route it automatically</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
          {/* Category */}
          <div>
            <label className="block text-[11px] uppercase tracking-widest font-semibold text-zinc-400 mb-2">Category *</label>
            ...
            {selectedCat?.description && (
              <p className="text-[11px] text-zinc-400 mt-1.5 flex items-start gap-1.5">
                <Info size={11} className="mt-0.5 flex-shrink-0" />{selectedCat.description}
              </p>
            )}
          ...
          {/* Subject */}
          <div>
            <label className="block text-[11px] uppercase tracking-widest font-semibold text-zinc-400 mb-2">Subject *</label>
          ...
          {/* Priority */}
          <div>
            <label className="block text-[11px] uppercase tracking-widest font-semibold text-zinc-400 mb-2">Priority</label>
            <div className="grid grid-cols-4 gap-2">
              {Object.entries(PRIORITY).map(([key, cfg]) => (
                <button key={key} type="button" onClick={() => set("priority", key)}
                  className={`py-2 rounded-xl border text-xs font-semibold transition ${form.priority === key
          ...
          {/* Summary */}
          <div>
            <label className="block text-[11px] uppercase tracking-widest font-semibold text-zinc-400 mb-2">Description *</label>
          ...
          {/* Attachments */}
          <div>
            <label className="block text-[11px] uppercase tracking-widest font-semibold text-zinc-400 mb-2">Attachments</label>
            <div onClick={() => fileRef.current?.click()}
              ...
              <p className="text-sm text-zinc-500"><span className="font-semibold text-zinc-700">Click to upload</span> or drag & drop</p>
            </div>
            <input ref={fileRef} type="file" multiple className="hidden" onChange={(e) => addFiles(e.target.files)} />
            {files.length > 0 && (
              <div className="mt-2 space-y-1.5">
                {files.map((f, i) => (
                  <div key={i} className="flex items-center justify-between bg-zinc-50 border border-zinc-100 rounded-xl px-3 py-2">
                    <div className="flex items-center gap-2 min-w-0">
                      <FileText size={12} className="text-zinc-400 flex-shrink-0" />
                      <span className="text-xs text-zinc-700 truncate">{f.name}</span>
                      <span className="text-xs text-zinc-400 flex-shrink-0">({(f.size / 1024).toFixed(0)} KB)</span>
                    </div>
        ...
        <div className="px-6 py-4 border-t border-zinc-100 flex gap-3 bg-zinc-50">
          <button onClick={submit} disabled={saving}
            ...
            {saving ? <><Loader2 size={13} className="tk-spin" />Submitting…</> : <><Ticket size={13} />Submit Ticket</>}
          </button>
          <button onClick={onClose} disabled={saving}
...
function DetailDrawer({ ticketId, onClose, onUpdated, addToast }) {
  ...
  const handleDownload = async (att) => {
    ...
    try { await tkDownload(TICKET_API.downloadAtt(att.id), att.file_name); }
    catch (e) { addToast(e.message, "error"); }
  ...
  return (
    <>
      <div className="fixed inset-0 z-[200] bg-black/50 backdrop-blur-sm tk-fade-in" onClick={onClose} />
      <div className="fixed right-0 top-0 bottom-0 z-[201] w-full sm:w-[500px] bg-white shadow-2xl flex flex-col tk-slide">
        ...
        <div className="px-6 py-5 border-b border-zinc-100 bg-zinc-950 flex items-start justify-between gap-4">
          <div className="min-w-0">
            ...
            {ticket && <p className="text-xs text-zinc-400 mt-1">#{ticket.id} · {ticket.category_name || "—"}</p>}
          </div>
        ...
        {loading ? (
          ...
          <div className="flex-1 overflow-y-auto">
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  <div key={label} className="bg-zinc-50 rounded-xl p-3 border border-zinc-100">
                    <p className="text-[10px] uppercase tracking-widest font-semibold text-zinc-400 mb-1">{label}</p>
            ...
            {/* Description */}
            <div className="px-6 py-4 border-b border-zinc-100">
              <p className="text-[11px] uppercase tracking-widest font-semibold text-zinc-400 mb-2">Description</p>
              <p className="text-sm text-zinc-700 leading-relaxed whitespace-pre-wrap">{ticket.summary}</p>
            </div>
            ...
            {atts.length > 0 && (
              <div className="px-6 py-4 border-b border-zinc-100">
                ...
                <div className="space-y-2">
                  {atts.map((a) => (
                    <div key={a.id} className="flex items-center justify-between bg-zinc-50 border border-zinc-100 rounded-xl px-3 py-2.5">
                      <div className="flex items-center gap-2 min-w-0">
                        <Paperclip size={12} className="text-zinc-400 flex-shrink-0" />
                        <span className="text-sm text-zinc-700 truncate">{a.file_name}</span>
                        {a.file_size && <span className="text-xs text-zinc-400 flex-shrink-0">({(a.file_size / 1024).toFixed(0)} KB)</span>}
                      </div>
                      <button onClick={() => handleDownload(a)} disabled={downloading === a.id}
                        ...
                        {downloading === a.id ? <Loader2 size={11} className="tk-spin" /> : <Download size={11} />}
            ...
            <div className="px-6 py-4">
              ...
              {comments.length === 0 ? (
                ...
                <div className="space-y-3 mb-4">
                  {comments.map((c, i) => (
                    <div key={c.id || i} className={`rounded-2xl p-3.5 ${c.is_hr ? "bg-violet-50 border border-violet-100" : "bg-zinc-50 border border-zinc-100"}`}>
                      <div className="flex items-center gap-2 mb-2">
                        <div className={`w-6 h-6 rounded-full flex items-center justify-center text-white text-[10px] font-bold flex-shrink-0
                          ${c.is_hr ? "bg-violet-600" : "bg-zinc-600"}`}>
                          {(c.author_name || "U")[0].toUpperCase()}
                        </div>
                        <span className="text-xs font-semibold text-zinc-700">{c.author_name || "User"}</span>
                        {c.is_hr && <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full bg-violet-100 text-violet-700">HR</span>}
                        <span className="text-[11px] text-zinc-400 ml-auto">{fmtFull(c.created_at)}</span>
                      </div>
              ...
              {ticket.status !== "CLOSED" && ticket.status !== "CANCELLED" && (
                <div className="flex gap-2">
                  ...
                  <button onClick={sendComment} disabled={sending || !comment.trim()}
                    ...
                    {sending ? <Loader2 size={14} className="tk-spin text-white" /> : <Send size={14} className="text-white" />}
                  </button>
                </div>
              )}
            </div>
        ...
        {ticket && ticket.status === "OPEN" && (
          <div className="px-6 py-4 border-t border-zinc-100">
            <button onClick={cancelTicket} disabled={cancelling}
              ...
              {cancelling ? <Loader2 size={13} className="tk-spin" /> : <XCircle size={13} />}
              Cancel Ticket
            </button>
          </div>
        )}
...
export default function MyTickets() {
  ...
  const catOptions = useMemo(() => [...new Set(tickets.map((t) => t.category_name).filter(Boolean))], [tickets]);
  ...
  return (
    <div className="tk-root min-h-screen bg-zinc-50">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex items-start justify-between mb-8 gap-4">
          <div>
            ...
            <p className="text-sm text-zinc-500 mt-1">Track and manage your support requests</p>
          </div>
        ...
        <div className="grid grid-cols-3 gap-4 mb-7">
          {[
            { label: "Open",        value: counts.open,       color: "from-blue-500 to-blue-700",     textColor: "text-blue-50",   sub: "text-blue-200" },
            { label: "In Progress", value: counts.inProgress, color: "from-violet-500 to-violet-700", textColor: "text-violet-50", sub: "text-violet-200" },
            { label: "Resolved",    value: counts.resolved,   color: "from-emerald-500 to-teal-600",  textColor: "text-emerald-50",sub: "text-emerald-200" },
          ].map(({ label, value, color, textColor, sub }) => (
            <div key={label} className={`bg-gradient-to-br ${color} rounded-2xl p-5 text-white shadow-sm`}>
              <p className={`text-xs font-semibold uppercase tracking-widest ${sub}`}>{label}</p>
        ...
        <div className="bg-white rounded-2xl border border-zinc-100 shadow-sm overflow-hidden">
          <div className="px-5 py-4 border-b border-zinc-100 flex flex-wrap items-center gap-2.5">
            ...
            {[
              { label: "Status", value: statusFilter, onChange: setStatus,
                opts: [["All","All Status"], ...Object.entries(STATUS).map(([k,v]) => [k, v.label])] },
              { label: "Priority", value: priorityFilter, onChange: setPriority,
                opts: [["All","All Priority"], ...Object.entries(PRIORITY).map(([k,v]) => [k, v.label])] },
              { label: "Category", value: categoryFilter, onChange: setCategory,
                opts: [["All","All Categories"], ...catOptions.map((n) => [n, n])] },
            ...
            {(search || statusFilter !== "All" || priorityFilter !== "All" || categoryFilter !== "All") && (
              <button onClick={() => { setSearch(""); setStatus("All"); setPriority("All"); setCategory("All"); }}
            ...
            <span className="ml-auto text-xs text-zinc-400">{filtered.length} ticket{filtered.length !== 1 ? "s" : ""}</span>
          </div>
          ...
          {paged.length === 0 ? (
            <div className="text-center py-20">
              ...
              <h3 className="text-base font-semibold text-zinc-800 mb-1">No tickets found</h3>
              <p className="text-sm text-zinc-500">
                {search || statusFilter !== "All" ? "Try adjusting your filters." : "Submit your first ticket to get started."}
              </p>
                ...
                <thead>
                  <tr className="border-b border-zinc-100 bg-zinc-50/60">
                    {["#", "Subject", "Category", "Priority", "Status", "Created", ""].map((h) => (
                      <th key={h} className="px-5 py-3 text-left text-[11px] font-bold text-zinc-400 uppercase tracking-widest whitespace-nowrap">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody className="divide-y divide-zinc-50">
                  {paged.map((t, i) => (
                    <tr key={t.id}
                      ...
                      <td className="px-5 py-4 text-xs text-zinc-400 font-mono">#{t.id}</td>
                      <td className="px-5 py-4 max-w-[200px]">
                        <p className="text-sm font-semibold text-zinc-800 truncate">{t.subject}</p>
                        {t.summary && <p className="text-[11px] text-zinc-400 truncate mt-0.5">{t.summary.slice(0, 60)}…</p>}
                      </td>
                      ...
                      <td className="px-5 py-4 text-xs text-zinc-400 whitespace-nowrap">{fmt(t.created_at)}</td>
                      <td className="px-5 py-4">
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between px-5 py-3.5 border-t border-zinc-100">
              ...
              <div className="flex items-center gap-1.5">
                ...
                {pageNums.map((n, i) =>
                  n === "..." ? (
                    <span key={`el-${i}`} className="w-8 h-8 flex items-center justify-center text-xs text-zinc-400">…</span>
                  ) : (
      ...
      {detailId && (
        <DetailDrawer ticketId={detailId}
