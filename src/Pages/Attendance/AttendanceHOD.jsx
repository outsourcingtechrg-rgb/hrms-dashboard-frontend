...
function getAuthFromToken() {
  try {
    ...
    return {
      employee_id: payload.sub || payload.id || payload.employee_id || null,
...
export default function EmployeesAttendancePage() {
  ...
  const fetchEmployees = useCallback(async () => {
    ...
    try {
      ...
      const allEmps = Array.isArray(data) ? data : data.employees || [];
      setEmployees(filterEmployeesByDepartment(allEmps, auth));
    } catch (e) {
  ...
  const fetchRecords = useCallback(async () => {
    ...
    try {
      ...
      const deptParam = getDepartmentQueryParam(auth);
      const url = `${attBase}/records?month=${month}${deptParam}&limit=500`;
  ...
  const fetchSummary = useCallback(async () => {
    ...
    try {
      const attBase = `${API.AllAttendance}/admin`;
      const deptParam = getDepartmentQueryParam(auth);
      const url = `${attBase}/summary?month=${month}${deptParam}`;
      const res = await fetch(url, { headers: getHeaders() });
      if (!res.ok) throw new Error(res.status);
  ...
  const openSidebar = useCallback(
    async (empDbId) => {
      ...
      try {
        const attBase = `${API.AllAttendance}/admin`;
        const deptParam = getDepartmentQueryParam(auth);
        const url = `${attBase}/records?employee_id=${empDbId}&month=${month}${deptParam}`;
        const res = await fetch(url, { headers: getHeaders() });
        if (res.ok) {
          const data = await res.json();
